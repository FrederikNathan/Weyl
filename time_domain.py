...
def get_steadystate(t0,dt,U_array,Rgrid):
    ...
    for s in svec:
        ...
        ind1 = int(((s)/T1)%1 *Nphi)
        ind2 = int(((s)/T2)%1 *Nphi)
        
        ind1_m = int(((s-dt)/T1)%1 *Nphi)#        print(t0)
        ind2_m = int(((s-dt)/T2)%1 *Nphi)#        print(t0)
 
   
        R0  += U0@Rgrid[ind1,ind2]@U0.conj().T*dt*exp(-(t0-s+dt/2)/tau)/tau
#        T0 += dt*exp(-(t0-s+dt/2)/tau)/tau
        if isnan(R0[0,0]):
            raise ValueError
         
        dU = U_array[ind1_m,ind2_m].conj().T
        U0 = U0@dU
...
R0,R0list,A0 =get_steadystate(0,dt,U_array,Rgrid)
R = 1*R0
Rlist = []
print("Evolving forward")
nt = 0
Out = 1
for t in arange(0,(1.1)*dt,dt):
    nt+=1
    (ind1,ind2)=(int(t*x*Nphi/ (2*pi))%Nphi for x in (omega1,omega2))
    (ind1_p,ind2_p)=(int(t*x*Nphi/ (2*pi))%Nphi for x in (omega1,omega2))
    
#    print(ind1)
    dU = U_array[ind1,ind2]
    R = dU@R@(dU.conj().T)
    R = exp(-dt/tau)*R + dt/tau*Rgrid[ind1_p,ind2_p]
...
#
R1 = get_steadystate(t,dt,U_array,Rgrid)[0]
print("Answer")
print(R)
print("")
print("Actual answer")
