...
import os 
NUM_THREADS = 1
RELATIVE_DT_MAX = 100    # Maximum dt relative to 1/|H|.
...
#    def compute_power(self,tlist,R):
...
#        self. = []
        ...
        self.Work_1 = zeros((nfreqs,nmat))
        self.Work_2 = zeros((nfreqs,nmat))
        self.E=zeros((nfreqs,nmat))          
...
#            self.Out += sum(exp(1j*freqlist*t)*self.R,axis=1)*self.dt/nmat
            self.dhdt_1,self.dhdt_2 = self.get_dhdt_12(t)
            self.h =self.get_h(t)

            self.Out += sum(exp(1j*freqlist*t)*self.R,axis=1)*self.dt/nmat       
            self.dW1 =real(trace(self.dhdt_1@self.R,axis1=2,axis2=3))*self.dt
            self.dW2 =real(trace(self.dhdt_2@self.R,axis1=2,axis2=3))*self.dt       
            self.dE = real(trace(self.h@self.R,axis1=2,axis2=3))*self.dt
...
#            
            ...
            self.E+=self.dE
            self.Work_2 += self.dW2
            self.Work_1 += self.dW1 
...
#                print(f"t mod T1 : {((t[0,0,0,0]%T1)/T1)}")
#                print(mean(self.dW1)/self.dt)
#                print(mean(self.dW2)/self.dt)
                self.W1_list.append(mean(self.dW1)/self.dt)
                self.W2_list.append(mean(self.dW2)/self.dt)

                Outlist.append(self.Out/(delta_t))
   
        self.Out = self.Out/(n_T1*self.T1)
        print(f"Done. Time spent: {B.toc(n=17,disp=0):.4} s")
        self.power1 = self.Work_1/(self.n_T1*self.T1)
        self.power2 = self.Work_2/(self.n_T1*self.T1)
        self.Eav = self.E/(self.n_T1*self.T1)
        t_out  =  self.T1 * nmat * n_T1
        
        return self.Out,t_out,mean(self.power1),mean(self.power2),mean(self.Eav)
    
    def get_h(self,t):
        
        self.h = wl.get_h_fourier_component(0,0,self.k)*(1+0*t)
#        print(self.h)
        self.h+=+wl.get_h_fourier_component(0,1,self.k)*exp(-1j*omega2*t)
#        print(self.h)       
        self.h+=+wl.get_h_fourier_component(0,-1,self.k)*exp(1j*omega2*t)
#        print(self.h)        
        self.h+=+wl.get_h_fourier_component(1,0,self.k)*exp(-1j*omega1*t)
#        print(self.h)        
        self.h+=+wl.get_h_fourier_component(-1,0,self.k)*exp(1j*omega1*t)
#        print(self.h)
        return self.h
    def get_dhdt_12(self,t):
        self.dhdt_mode1 = (wl.get_dhdt_fourier_component(1,0,self.k)*exp(-1j*omega1*t)+wl.get_dhdt_fourier_component(-1,0,self.k)*exp(1j*omega1*t))
        self.dhdt_mode2 = (wl.get_dhdt_fourier_component(0,1,self.k)*exp(-1j*omega2*t)+wl.get_dhdt_fourier_component(0,-1,self.k)*exp(1j*omega2*t))
        
        
        return self.dhdt_mode1,self.dhdt_mode2
...
if __name__=="__main__":
    ...
    Out = S.get_ft([0,omega2,omega1],10*T1)
