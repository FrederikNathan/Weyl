...
if True:   
    ...
    nlist = [1,24,10]+list(arange(2,10))+list(arange(11,17))+list(arange(18,26))
    # nlist.pop(0)
    # nlist=[1,2]
    # try:
        # 1*tw
...
for ns in range(0,NS):
    ...
    p1  = P1[ind]   
    p2  = P2[ind]   
    s1  = std1[ind] / (Joule/(second_physical*micrometer**3))
    s2  = std2[ind] / (Joule/(second_physical*micrometer**3))
    ...
    figure(1)
    errorbar(tau/picosecond_physical,p2/(1e3*Joule/(second_physical*millimeter**3)),yerr=s2,fmt="o-",capsize=4,lw=1.2,markersize=4,color=sns.color_palette()[ns])

    figure(2)
    errorbar(tau/picosecond_physical,p1/(1e3*Joule/(second_physical*millimeter**3)),yerr=s1,fmt="o-",capsize=3)
    legendlist.append("\t$v_{\\rm F} = "+f"{series_list[ns,2]/(1e6*meter/second):.2}$, \t$\mu={series_list[ns,-2]:.4}$, \t$\omega_2/\omega_1 = {omega2/omega1:.9}$")


figure(1)
# legend(l¨egendlist)
plot([0,1500],[0,0],'--',color="dimgray",linewidth=1)
xlabel("relaxation time, $\\tau$, picoseconds")
ylabel("Amplification power, $kW/mm^3$")
