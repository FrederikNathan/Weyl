...
def get_steady_state(k,freq1_list,mode ="full",mu=0,Nphi=200,NP0=10,convergence_treshold=1e-5,return_convergence_parameters=False,NPmax=100,plot_progress=False):
    """
...
#    global S,steady_state,block_list
    while not NP_converged:
...
#        global nv1
        ...
        if mode == "approx":
            ...
            R1 = FR1.reshape((NP1,NP2,2,2))
            rho1 = R1[:,:,0,0] + R1[:,:,1,1]

            return rho0,rho1,rho2
        elif mode!="full":
            raise ValueError("mode must be 'full' or 'approx'")
        ...
        if convergence_ratio <convergence_treshold:
            ...
            NPnew=max(NP1+1,int(NP1*(2**0.25)))
            ...
            if NPnew>=NPmax:
                print(f"{'-'*80}")
                print("Reached maximum bound for NP1! Exiting")
            ...
            print("")
            print(f"No convergence yet. ")
            print(f"    Increasing NP1,NP2 to {NP1},{NP2}.")
            print(f"Convergence ratio is {convergence_ratio}")
    ...
    if return_convergence_parameters:
        
        return rho0,rho1,rho2,(Convergence,NP1,NP2,convergence_ratio)
    else:
        return rho0,rho1,rho2
...
def solve_weyl(k,mode="full",mu=0,Nphi=200,NP0=10,convergence_treshold=5e-5,NPmax=100):
    """
    ...
    if mode=="full":
        ...
        rho0,rho1,rho2,(Conv,NP1,NP2,cr) = get_steady_state(k,[0,1],mu=mu,Nphi=Nphi,NP0=NP0,convergence_treshold=convergence_treshold,return_convergence_parameters=True,NPmax=NPmax,plot_progress=1)
    ...
    return density,power_1,power_2,J,Conv,NP1,NP2,cr
...
def main_run(klist,parameterlist,Plot=False,Save=True,SaveFig=True,PreStr="",display_progress=10,savetime=10*3600,NP0=20,convergence_treshold=5e-4,tol=1e-4):
    """
    Main sweep.
    ...
    for n in range(0,Nk):
        ...
        set_parameters(parameters[:-1])
        NPmax = parameters[-1]
        
        density,P1,P2,J,Conv,NP1,NP2,convergence_ratio = solve_weyl(k,mu=Mu,NPmax=NPmax,NP0=NP0,convergence_treshold=convergence_treshold)
        gc.collect()
        ...
        convlist[n,:] =array([Conv,NP1])
        crlist[n] = convergence_ratio

        if B.toc(n=1,disp=False)>savetime and Save:
            dissipation_list = -(P1list+P2list)

            savez(DataPath,parameterlist=parameterlist[:n+1],klist=klist[:n+1],P1list=P1list[:n+1],P2list=P2list[:n+1],Dissipation=Dissipation[:n+1],density_list=density_list[:n+1],convlist = convlist[:n+1,:],crlist=crlist[:n+1])
            B.tic(n=1)
    
    dissipation_list = -(P1_list+P2_list)
    
    if Save:
        savez(DataPath,parameterlist=parameterlist,klist=klist,P1_list=P1_list,P2_list=P2_list,dissipation_list=dissipation_list,density_list=density_list,convlist = convlist,crlist=crlist)
...
if __name__=="__main__":
    ...
    # K grid
    # =============================================================================
    kzlist = linspace(-0.3,0.2,20)#,0.2,20)*(Å**-1)
    kx=0.0
    ky=0
...
#    S = shape()
    parameters = 1*array([[omega1,omega2,tau,vF,V0x,V0y,V0z,EF1,EF2,Mu,Temp,NPmax]]*len(klist))
#,V0list,ROlist
    V0list =[]
    ROlist = []   
    Rlist = []
    RFlist = []     
    P1_list,P2_list,dissipation_list,density_list,convlist,crlist=main_run(klist,parameters,Plot=0,Save=True,display_progress=10,NP0=30,convergence_treshold=1e-3,tol=1e-4)
